...
void Enemy::Updete()
{
	...
	postooo.x = (float)Map_->Getmappos(
		posrecord[roadMaxcount - (int)run].ispos[0],
		posrecord[roadMaxcount - (int)run].ispos[1]).x + Map_->GetMapSize().x/2;
	postooo.y = (float)Map_->Getmappos(
		posrecord[roadMaxcount - (int)run].ispos[0],
		posrecord[roadMaxcount - (int)run].ispos[1]).y + Map_->GetMapSize().y / 2;

	enemySprite->SetPosition(postooo);
	saiki_num = 0;
	worldTransform_.translation_ = Vector3{postooo.x-2.5f,postooo.y-2.5f,-40};
